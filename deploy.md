...
- 注册账号（学号必须 11 位数字）
- 管理员登录后强制重设密码
- 浏览公开首页（`/`）和社团详情页（`/club/:id`）
- 创建社团（含封面图）、创建活动、走审批、录入财务
- 创建个人待办、发布社团任务（含多负责人）
- 验证管理后台路由（`/admin/*`）均需登录

### 7.4 数据库迁移验证

- `npm run db:migrate:status` 应显示全部 12 个迁移为 `up`
- 检查 `personal_tasks`、`club_tasks` 表是否存在
- 检查 `clubs` 表是否包含 `cover_image_url` 列
